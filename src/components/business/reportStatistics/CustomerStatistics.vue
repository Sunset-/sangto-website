    ...
    <sunset-layout title="报告统计">
        <div class="hospital-statistics-container">
            ...
            <div>
                ...
                <sunset-toolbar style="padding-left:10px;padding-bottom:10px;" :options="detailTableOptions.toolbar"></sunset-toolbar>
    ...
    export default {
        methods: {
            refresh(filter) {
                ...
                this.currentFilter.customerId = filter.customerId;
                this.currentFilter.startTime = filter.startTime;
                this.currentFilter.endTime = filter.endTime;
                this.detail();
            },
            detail(type, expert, taskProperty) {
                var filter = this.currentFilter;
                filter.deviceType = type;
                filter.teamId = expert && expert.expertId;
            ...
            active(type, expert, taskProperty) {
                var filter = this.currentFilter;
                return filter.deviceType == type && filter.teamId == (expert && expert.expertId) && filter.taskProperty ==
                    taskProperty;
            }
        },
        data() {
            return {
                ...
                filterOptions: {
                    ...
                    fields: [{
                        ...
                        changeFilter: true,
                        clearable: false,
                        default: [new Date(new Date().setDate(1)), new Date()],
                        shortcuts: [{
                                text: '今天',
                                value() {
                                    var now = new Date();
                    ...
                    format: function (filter) {
                        if (filter.date && filter.date[0] && filter.date[1]) {
                            filter.startTime = Sunset.Dates.format(filter.date[0], 'yyyy-MM-dd 00:00:00');
                            filter.endTime = Sunset.Dates.format(filter.date[1], 'yyyy-MM-dd 23:59:59');
                        } else {
                            var date = new Date();
                            filter.endTime = Sunset.Dates.format(date, 'yyyy-MM-dd 00:00:00');
                            date.setDate(1);
                            filter.startTime = Sunset.Dates.format(date, 'yyyy-MM-dd 23:59:59');
                        }
                        delete filter.date;
                        return filter;
                    }
                ...
                detailTableOptions: {
                    columns: [{
                        ...
                        },
                        {
                            ...
                            name: 'reportFinishTime',
                            format(v, record) {
                                var time = parseInt(((v || new Date()) - record.claimTime) / 1000),
